...
from scipy import ones, floor, array, sqrt, log, ceil, cov
...
def _web_test_agent( refm_call, agent_call, rflip, episode_length, \
                 disc_rate, stratum, program ):
    # create reference machine
    refm = eval( refm_call )
...
# Simple MC estimator, useful for checking the more complex adaptive estimator.
# It doesn't do logging as the log file assumes dual runs for antithetic variables.
def web_simple_mc_estimator( refm_call, agent_call, episode_length, disc_rate, \
                         sample_size ):
    ...
    for stratum, program in sample_data:
        rflip = choice([-1,1])
        perf = _web_test_agent( refm_call, agent_call, rflip, episode_length, disc_rate, \
                            stratum, program )[1]
        if not isnan(perf):
            result[i] = perf
            i += 1
...
def load_samples( refm, cluster_node, simple_mc ):
    ...
    program_sample_filename = dirname(__file__) + "/refmachines/samples/" + str(refm) \
